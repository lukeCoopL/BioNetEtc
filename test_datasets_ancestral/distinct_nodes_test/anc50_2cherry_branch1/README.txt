...
for i in range(6):
    if i in leaves:
        leafGraphs[i]= nx.read_edgelist("LEAF"+str(i)+"_two_cherry_branchLength1.txt",create_using=nx.DiGraph)
    else:
        internalGraphs[i]= nx.read_edgelist("INTERNAL"+str(i)+"_two_cherry_branchLength1.txt",create_using=nx.DiGraph)